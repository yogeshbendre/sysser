...
def findvSanHealthLevel1(vcName,mymin):
    ...
    if epochtime is not None:
        currTime = dt.fromtimestamp(float(epochtime))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t","--timestamp", type=str,help="Specify linux epoch time for reporting. Default: current time")
    args = parser.parse_args()
    
    if args.timestamp:
        epochtime = str(args.timestamp)
